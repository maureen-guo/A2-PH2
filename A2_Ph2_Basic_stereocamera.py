...
def main():
    ...
    W3_points_left = pd.read_excel('./A2_Ph2/W3/Left/W3_Cube_Left.xlsx')
    W3_points_right = pd.read_excel('./A2_Ph2/W3/Right/W3_Cube_Right.xlsx')
    ...
    print("RMSE_intersection:", rmse_value)

    #W3 image
    xu_W3_left = W3_points_left['u\''].values
    xu_W3_right = W3_points_right['u\''].values
    yu_W3_left = W3_points_left['v\''].values
    yu_W3_right = W3_points_right['v\''].values

    X_left = W3_points_left['X'].values
    Y_left = W3_points_left['Y'].values
    Z_left = W3_points_left['Z'].values
    X_right = W3_points_right['X'].values
    Y_right = W3_points_right['Y'].values
    Z_right = W3_points_right['Z'].values

    W3_image_left = cv2.imread('./A2_Ph2/W3/Left/W3_Cube_Left.png')
    W3_image_right = cv2.imread('./A2_Ph2/W3/Right/W3_Cube_Right.png')

    h1, w1 = W3_image_left.shape[:2] 
    cx3 = w1 / 2
    cy3 = h1 / 2

    h2, w2 = W3_image_right.shape[:2] 
    cx4 = w2 / 2
    cy4 = h2 / 2

    W3_pixel_size = (W3_pixel_size_x + W3_pixel_size_y) / 2
    dx = dy = W3_pixel_size

    xu_left, yu_left = A2_uti.calculate_undistorted_coordinates(xu_W3_left, yu_W3_left, cx3, cy3, W3_pixel_size_x, W3_pixel_size_y,sx=1)
    xu_right, yu_right = A2_uti.calculate_undistorted_coordinates(xu_W3_right, yu_W3_right, cx4, cy4, W3_pixel_size_x, W3_pixel_size_y,sx=1)

    # Select the farthest reference point for W3
    idx_h = np.argmax(np.sqrt(xu_left**2 + yu_left**2))
    X_ref_w, Y_ref_w, Z_ref_w = X_left[idx_h], Y_left[idx_h], Z_left[idx_h]
    x_ref_w, y_ref_w = xu_left[idx_h], yu_left[idx_h]
    ...
    r_values, tx, ty, tz, f_left, xu_left, yu_left= A2_uti.calculate_matrices_and_focal_length(L_left, xu_W3_left, yu_W3_left, X_left, Y_left, Z_left, dx, dy, cx3, cy3, X_ref_w, Y_ref_w, Z_ref_w, x_ref_w, y_ref_w)
    Rt_left = A2_uti.create_matrices(r_values, tx, ty, tz)
    ...
    # calculate the error in cube of left image of W3
    rmse_value = A2_uti.calculate_rmse(original_points1, projected_points1)
    print("RMSE1 in cube in W3:", rmse_value)


    #right image
    # Select the farthest reference point for W3
    idx_w = np.argmax(np.sqrt(xu_right**2 + yu_right**2))
    X_ref_w_right, Y_ref_w_right, Z_ref_w_right = X_right[idx_w], Y_right[idx_w], Z_right[idx_w]
    x_ref_w_right, y_ref_w_right = xu_right[idx_w], yu_right[idx_w]

    r_values, tx, ty, tz, f_right, xu_right, yu_right = A2_uti.calculate_matrices_and_focal_length(L_right, xu_W3_right, yu_W3_right, X_right, Y_right, Z_right, dx, dy, cx4, cy4, X_ref_w_right, Y_ref_w_right, Z_ref_w_right, x_ref_w_right, y_ref_w_right)
    Rt_right = A2_uti.create_matrices( r_values, tx, ty, tz)
    ...
    #calculate error in cube of right image of W3
    rmse_value = A2_uti.calculate_rmse(original_points2, projected_points2)
    print("RMSE2 in cube in W3:", rmse_value)
    ...
    # calculate intersections in W3
    intersections = A2_uti.compute_intersections(OcW_left, MwI_left, OcW_right, MwI_right)
    A2_uti.plot_projection_results(original_left2, np.array(intersections[:70]), original_right2, np.array(intersections[70:]))

    original_points = list(zip(X_left, Y_left, Z_left))
    rmse_value = A2_uti.calculate_rmse(original_points, intersections)
    print("RMSE_intersection in W3:", rmse_value)
